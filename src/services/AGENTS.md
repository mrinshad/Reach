...
| [`easy_apply_service.py`](file:///Users/apple/Byten/linkedInScrapper/src/services/easy_apply_service.py) | LinkedIn Easy Apply automation engine: batch crawler (`f_AL=true`), multi-step modal question inspector, contact/resume prefill, dummy input screening collection, and direct submission. | `run_easy_apply_crawler`, `apply_to_single_easy_apply_post`, `execute_easy_apply`, `inspect_easy_apply_modal`, `dismiss_easy_apply_modal` | Playwright, `firefox_connector.py`, `src/db/posts.py`, `config.py` |
