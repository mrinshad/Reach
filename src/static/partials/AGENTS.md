...
| **`tab_easy_apply.html`** | [`tab_easy_apply.html`](file:///Users/apple/Byten/linkedInScrapper/src/static/partials/tab_easy_apply.html) | LinkedIn Easy Apply workspace (`#tabEasyApply`), crawler header form, 4-stat KPI ribbon, separated filter pills & search box, floating batch action banner (`#easyBatchToolbar`), data table with fixed layout and status dropdowns (`#easyApplyTableBody`), and details modal (`#modalEasyDetails`) with status toggles. |
