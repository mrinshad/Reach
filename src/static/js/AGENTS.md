...
| [`easy_apply.js`](file:///Users/apple/Byten/linkedInScrapper/src/static/js/easy_apply.js) | `fetchEasyApplyPosts`, `filterEasyApplyStatus`, `handleEasyTableSearch`, `triggerEasyApplyCrawl`, `triggerSingleEasyApply`, `toggleEasyStatusMenu`, `setEasyPostStatus`, `batchSetEasyStatus`, `setModalPostStatus`, `openEasyDetailsModal`, `closeEasyDetailsModal`, `openScreeningModal`, `closeScreeningModal`, `openBulkSearchModal`, `closeBulkSearchModal`, `handleBulkCsvUpload`, `handleBulkKeywordsInput`, `submitBulkEasySearch`, `parseScreeningQuestions`, `triggerScreeningRescan`, `markScreeningAsReady`, `copyScreeningLink`, `copyEasyJobLink`, `batchCopyEasyLinks`, `batchDismissEasyJobs`, `batchApplySelectedEasyJobs`, `clearEasySelection` | LinkedIn Easy Apply & Direct Jobs workspace: crawling trigger, CSV bulk keywords importer, fixed-position smart status dropdowns, batch status transitions, selection reset, elevated details modal, dedicated Screening Questionnaire modal, and automated application dispatch. |
| [`history.js`](file:///Users/apple/Byten/linkedInScrapper/src/static/js/history.js) | `fetchSentPosts`, `setOthersFilter`, `loadRejectionReasonsFilter`, `revertPostToDraft`, `switchHistoryView`, `openActivityLogsView`, `setActivityLogTypeFilter`, `applyActivityLogStatusFilter`, `changeActivityLogsRowsPerPage`, `goToActivityLogsPage`, `fetchActivityLogs`, `renderActivityLogs`, `openRunLogsModal`, `closeRunLogsModal`, `copyRunLogs` | Sent & cancelled history table, dynamic rejection reason chips, history pagination, post draft restoration, activity & run log history table, task filtering, terminal output log modal. |
